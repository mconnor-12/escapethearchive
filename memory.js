...
function goToRoom(n) {
  ...
  const labels = [
    ...
    'Room V — Smooth Synthetic Coherence',
  ];
...
function initTooltips() {
  const tips = {
    ...
    '[data-tip="unearned"]':
      '"Unearned" means the signal looks authoritative but isn\'t — e.g. engagement counts, sponsored ranking, or a platform badge that certifies a domain, not content accuracy.',
  };
...
function saveAnnotations() {
  ...
  if (!a1) { showFeedback('fb-annot','warn','Please complete annotation 1 before saving.'); document.getElementById('annot-1')?.focus(); return; }
  if (!a2) { showFeedback('fb-annot','warn','Please complete annotation 2 before saving.'); document.getElementById('annot-2')?.focus(); return; }
  if (!a3) { showFeedback('fb-annot','warn','Please complete annotation 3 before saving.'); document.getElementById('annot-3')?.focus(); return; }

  S.r0.annotationsSaved = true;
  document.getElementById('annot-save-btn')?.setAttribute('disabled', true);
  showFeedback('fb-annot', 'ok', '✓ Annotations saved. Scroll down to examine the three hotspots in the exhibit brief — click all three to continue.');
...
function checkPrologueUnlock() {
  if (S.r0.annotationsSaved && S.r0.hotspot1 && S.r0.hotspot2 && S.r0.hotspot3) {
    ...
    showToast('✓ All hotspots examined — proceed to Room I when ready.');
...
function placeInZone(zone, val) {
  ...
  zone.setAttribute('aria-label', 'Contains: ' + val);
  _euphDndState[zoneKey] = val;
  checkAllEuph();
}

function checkAllEuph() {
  const correct = Object.entries(EUPH_ANSWERS).every(([k, ans]) => _euphDndState[k] === ans);
  if (correct) {
    S.r1.euphDone = true;
    S.r1._euphDnd = true;
    ...
    if (filled > 0 && filled < 3) {
      showFeedback('fb-euph', 'info', filled + ' of 3 placed. Look closely — which official phrase most precisely conceals each plain-language term?');
    }
  }
}
...
function checkEuph() {
  ...
  if (a === 'systematic destruction' && b === 'forced removal' && c === 'administrative delay') {
    ...
    showFeedback('fb-euph', 'warn', 'Not quite. Re-examine the official language — which term does each phrase most precisely conceal?');
  }
}
...
function submitLeadJudgment() {
  ...
  if (!judgment || judgment.length < 20) {
    showFeedback('fb-r1', 'warn', 'Your judgment must be substantive — at least one full sentence addressing specific claims in the record.');
...
function tagSource(el, sourceId, trust) {
  ...
  // FIX: Count each source once — allow re-rating but don't double-count
  if (!_r2Tagged[sourceId]) {
    S.r2.sourcesTagged++;
...
function submitEchoTrace() {
  ...
  if (!trace || trace.length < 15) {
    showFeedback('fb-r3', 'warn', 'Trace the full citation path before rendering a verdict — name the specific chain.');
...
function submitLeadJudgmentR3(judgment) {
  ...
  const feedbacks = {
    ...
    'independently corroborated': `Verdict recorded: <strong>Independently corroborated</strong>. Consider re-examining the chain — multiple outlets cited the same AP wire, which cited the same WRA press release. Repetition across outlets is not independent corroboration; it is distribution of a single source.`,
...
function rateSignal(el, sigId, verdict) {
  ...
  // FIX: Count each signal once
  if (!_r4Rated[sigId]) {
    S.r4.signalsRated = Object.keys(_r4Rated).length + 1;
    const counter = document.getElementById('r4-rated-count');
    if (counter) counter.textContent = S.r4.signalsRated;
    logEvent('signal_rated', { sig: sigId, verdict, room: 4 });
  }
...
function submitFeedReflection() {
  ...
  if (!ref || ref.length < 20) {
    showFeedback('fb-r4', 'warn', 'Reflect more fully — at least two sentences on how ranking shaped your judgment.');
...
const FRUST_MSGS = {
  1: 'The record is designed to be insufficient — that\'s the point. You\'re not failing to find evidence; you\'re experiencing what curated scarcity feels like as an analytical condition.',
  ...
  3: 'Echo corroboration is designed to look like consensus. That it deceived you is not a failure — it\'s exactly what the exercise is built to surface.',
  4: 'The feed is engineered to make low-quality sources feel authoritative. Noticing that discomfort is the analytical move.',
  5: 'The AI summary is smooth by design. Your job isn\'t to improve it — it\'s to name what it made disappear.',
};

function recoverFrustrationDocument(room) {
  showFeedback('fb-r' + room + '-frust', 'info',
    '↩ <strong>Recovery note:</strong> ' + (FRUST_MSGS[room] || 'The archive hasn\'t changed — your reading of it has room to deepen.'));
...
function saveSheetURL() {
  ...
  if (!url) { document.getElementById('sheets-status').textContent = 'Paste a URL first.'; return; }
  localStorage.setItem(SHEETS_URL_KEY, url);
  document.getElementById('sheets-status').textContent = '✓ URL saved — annotations will sync to your Sheet.';
}

async function testSheetURL() {
  const url = localStorage.getItem(SHEETS_URL_KEY);
  const status = document.getElementById('sheets-status');
  if (!url) { if (status) status.textContent = '✗ No URL saved yet.'; return; }
  try {
    await fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'ping'}), mode:'no-cors' });
    if (status) status.textContent = '✓ Ping sent. Check your Sheet for the ping row.';
  } catch(e) {
    if (status) status.textContent = '✗ Connection error: ' + e.message;
  }
}
