...
function saveAnnotations() {
  ...
  showFeedback('fb-annot', 'ok', '✓ Annotations saved. Scroll back up and click all three bold phrases in the exhibit brief , click all three to continue.');
...
function initHotspots() {
  ...
  document.querySelectorAll('.hotspot-btn').forEach(btn => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!reveal) return;
      // Always open (never re-hide on second click — student stays oriented)
      reveal.hidden = false;
      reveal.removeAttribute('aria-hidden');
      btn.setAttribute('aria-expanded', 'true');
      btn.classList.add('visited');
      // Only log + check once per hotspot
      if (!S.r0['hotspot' + key]) {
